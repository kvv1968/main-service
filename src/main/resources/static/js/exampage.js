...
let oldEvent,
    ...
    myData = [{
        id: 1,
        start: '2022-01-01T13:00',
        end: '2022-01-01T13:45',
        title: '[ПРИМЕР] митап {Ваш Логин}',
        description: '[ПРИМЕР] Прошу Вас согласовать дату митапа по теме JAVA_1',
        allDay: false,
        free: true,
        color: '#009788'
    }],
...
let calendar = mobiscroll.eventcalendar('#demo-add-delete-event', {
...
let popup = mobiscroll.popup('#demo-add-popup', {
    display: 'bottom',
...
let range = mobiscroll.datepicker('#event-date', {
    controls: ['date'],
    select: 'range',
...
function selectColor(color, setColor) {
    ...
    let newSelected = document.querySelector('.crud-color-c[data-value="' + color + '"]');
...
function sendForApproval() {
    if(data.length !== 0){
        let tempEvents = data[0].description + "  " + data[0].title + "\r\n\r\n";

        for (let i = 0; i < data.length; i++) {
            tempEvents += "начало -> " + data[i].start + "\r\nконец -> " + data[i].end + "\r\n\r\n";
        }
        $.ajax({
            url: '/user/cal/events',
            method: 'POST',
        ...
        alert('Письма не отправлены, потому что Вы ничего не выбрали');
    }
    history.back();
}